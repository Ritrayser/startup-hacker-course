...
const saveBook = (data) => {
  ...
  if (editingBookId.value) {
    ...
    if (i !== -1) books.value[i] = { ...books.value[i], ...data }
  } else {
    books.value.push({ id: Date.now(), ...data, rating: 0 })
  }
...
const setRating = (bookId, stars) => {
  const book = books.value.find(b => b.id === bookId)
  if (book) book.rating = stars
}

const resetRatings = () => books.value.forEach(b => (b.rating = 0))

const handleHoverEnter = (bookId, star) => {
  ...
  <div class="app">
    ...
    <div class="books">
      <BookCard
        ...
        @edit="openEditForm"
        @delete="removeBook"
        @setRating="(stars) => setRating(book.id, stars)"
        @hoverEnter="(star) => handleHoverEnter(book.id, star)"
